...
namespace DataAccess.Crud {
    public class MembresiaCrudFactory : CrudFactory {
        ...
        MembresiasUsuarioMapper mapperU;

        public MembresiaCrudFactory() : base() {
            mapper = new MembresiaMapper();
            mapperU = new MembresiasUsuarioMapper();
        ...
        public void UpdateMembresiaUsuario(BaseEntity entity)
        {
            var Membresia = (MembresiasUsuario)entity;
            dao.ExecuteProcedure(mapperU.GetUpdateStatement(Membresia));
        }
        public T RetrieveMembresiaUsuario<T>(BaseEntity entity)
        {
            var sqlOperation = mapper.GetRetriveStatementMembresiUsuario(entity);
            var lstResult = dao.ExecuteQueryProcedure(sqlOperation);
            var dic = new Dictionary<string, object>();
            if (lstResult.Count > 0)
            {
